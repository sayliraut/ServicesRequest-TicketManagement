    ...
    @php
        ...
        $navItems = [
            ...
            [
                ...
                'route' => 'tickets.index',
                'icon'  => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6M9 16h6M9 8h6"/>',
                'pattern' => 'tickets.*',
                'extra' => '<a href="'.route('tickets.create').'" class="ml-1 inline-flex items-center justify-center w-8 h-8 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition duration-150 flex-shrink-0" title="Create new request"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg></a>',
            ],
        ];
        ...
        <nav class="space-y-1">
            @foreach($navItems as $item)
                ...
                <div class="flex items-center justify-between py-1">
                    <a href="{{ route($item['route']) }}" class="flex-1 flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition duration-150 {{ $active ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
